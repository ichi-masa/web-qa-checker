...
- ページの自動収集は **sitemap.xml** または **トップページのリンク** からのみ行います。どちらにも含まれないページ（LPや直リンクのないページ等）は、対話モードの「追加URL」で手動指定してください
- CSSアニメーションやトランジションがあるページでは、スクリーンショットが途中の状態で撮影されることがあります。スクショ撮影前にアニメーションを無効にしておくことを推奨します
- W3C バリデーションと Lighthouse は外部APIを使用するため、オフライン環境では実行できません
- W3C CSSバリデーションは同一オリジンのCSSのみ対象です。CDN配信のCSSはチェックされません。また、CSSが50KBを超える場合はスキップされます
- WPセキュリティチェックはWordPress以外のサイトでも実行されます（該当URLが404の場合は問題なしと判定）
- チェックにはChromeを使用しています。Safari・Firefoxでの表示差異は手動確認が必要です
